...
This repository builds **regulator** (formerly VSM-Pi): a coding-agent harness on [Pi](https://pi.dev/) with an explicit cybernetic control plane, applying Stafford Beer's Viable System Model (VSM) as a control architecture. regulator provides its own orchestrator. The product is `packages/regulator` (the control plane) with `packages/regulator-pi` (the Pi host); the course under `course/` builds it lesson by lesson and cites it there.
...
Build `regulator` through the course, one module per lesson, promoting its Pi-free pieces into `packages/protocol` and `packages/core` as they stabilize.
