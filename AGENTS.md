...
This repository builds **regulator** (formerly VSM-Pi): a coding-agent harness on [Pi](https://pi.dev/) with an explicit cybernetic control plane, applying Stafford Beer's Viable System Model (VSM) as a control architecture. regulator provides its own orchestrator. The product is `packages/regulator` (the control plane) with `packages/regulator-pi` (the Pi host); the course, Viable Agents (a separate private repository), builds it lesson by lesson and cites it by link; `docs/GLOSSARY.md`, `docs/PATHOLOGIES.md` and `docs/examples/` are the reference documents the two share.
