...
pi install ./course/lab             # every checkpoint, into user settings; -l for project settings
# or, for one session without installing:
pi -e ./course/lab/dist/cp2-typed-tools.js -e … -e ./course/lab/dist/cp12-algedonic.js
```

`pnpm --filter @metacoding/vsm-pi-course-lab regulator -- <command>` runs the CLI from
the workspace; a global link (`pnpm link --global` in `course/lab`) gives `regulator`
on the path.
...
regulator unit drive course/lab/contracts/fix-known-issue.json
```
...
| Where | What |
| --- | --- |
| `course/lab/settings.json` | the Pi settings a unit's session runs under, held in memory by the dispatcher; the project's `.pi/settings.json` is never read |
| `course/lab/policies/*.json` | budgets and model routes, recovery, routing, interaction (timeouts, attention, reminders, people) |
| `course/lab/profiles/*.json` | what a unit type may use and write |
...
4. Run the drift suite headlessly (`regulator eval course/lab/evals/drift.json
