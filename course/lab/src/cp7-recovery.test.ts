...
import { EffectJournal, ExecutionStore, effectKey } from "@metacoding/vsm-pi-core";
import { createRecoveryExtension, outboxHas } from "./cp7-recovery.js";
...
test("checkpoint 7 loads into a real Pi session and registers notify_owner with its effect declared", async (t) => {
  ...
  const loader = new DefaultResourceLoader({
    cwd, agentDir, settingsManager, additionalExtensionPaths: [fileURLToPath(new URL("./cp7-recovery.js", import.meta.url))],
  ...
  const { TOOL_EFFECTS } = await import("@metacoding/vsm-pi-core");
