/**
 * S2 mechanisms, Pi-free: leases with liveness, and the thrash detector.
 *
...
export class LeaseStore {
  ...
  async list(): Promise<Lease[]> {
    ...
    for (const file of files.filter((f) => f.endsWith(".json")).sort()) {
      leases.push(JSON.parse(await readFile(path.join(this.dir, file), "utf8")) as Lease);
    }
    return leases;
  }
  ...
  async get(unitId: string): Promise<Lease | undefined> {
    try {
      return JSON.parse(await readFile(this.#file(unitId), "utf8")) as Lease;
    } catch {
      return undefined;
    }
  }
