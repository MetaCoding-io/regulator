...
| **S3\* that reads what S1 wrote** — the auditor runs the suite the unit edited | `run_tests` passes on a branch that deleted the failing tests; the defect is closed as fixed | `inherited-tests-check` (M09, added after M15): the base's suite judges the unit's tree, and a shrunk inherited file fails | the **self-certifier** behaviour; the `no-inherited-tests` arm shows it closing green | an expectation rewritten to the current output inside an exempt file (`inherited-tests-check` limitation 1) |
