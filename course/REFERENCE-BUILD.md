...
- **Checkpoint files** under `course/lab/src/`, one per lesson, each loadable with
  `pi -e` and tested without a model. Checkpoints 0–12 exist. Their Pi-free modules have
  ...
  policy, deliveries and interactions.
- **The regulator registry** — schema and check ship in `packages/`; the lab CLI and
  thirty-two records live under `course/lab/registry/`; the control room is design only, but its
