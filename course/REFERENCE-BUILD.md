...
- **Checkpoint files** under `course/lab/src/`, one per lesson, each loadable with
  `pi -e` and tested without a model. Checkpoints 0–11 exist. Their Pi-free modules have
  ...
  memory commands in the CLI, and the drift scenario.
- **The regulator registry** — schema and check ship in `packages/`; the lab CLI and
  twenty-eight records live under `course/lab/registry/`; the control room is design only, but its
