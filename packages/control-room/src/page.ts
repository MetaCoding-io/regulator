...
(() => {
  ...
  function renderInstances(instances) {
    ...
    instances.forEach((inst, i) => {
      ...
      html += "<p class='banner'>" + (inst.manifest ? "definition " + esc(inst.manifest.definition.name) + " at " + esc(inst.manifest.definition.harnessRevision.slice(0, 7)) + " · " + inst.manifest.definition.registry + " regulators · pi " + esc(inst.manifest.definition.pi) + " · initialized " + esc(inst.manifest.initializedAt.slice(0, 10)) + " by " + esc(inst.manifest.initializedBy) + (inst.manifest.writablePaths ? " · writable " + inst.manifest.writablePaths.map(esc).join(", ") : "") + (inst.manifest.protectedPaths ? " · protected " + inst.manifest.protectedPaths.map(esc).join(", ") : "") : inst.manifestProblem ? "<span class='problem'>" + esc(inst.manifestProblem) + "</span>" : "no instance manifest: not initialized with <code>regulator init</code>") + "</p>";
  ...
  function renderInspector() {
    ...
    html += "<section><h2>Replay — the unit's records in time order</h2><p class='banner'>contract → attempts → tool executions → evidence → verdicts → decisions → obligations → deliveries; from the span projection, never the transcript</p>" +
      ((u.timeline || []).length ? "<table><tr><th>at</th><th>what</th><th>status</th><th>regulator</th><th>detail</th></tr>" + u.timeline.map((e) => "<tr><td class='mono'>" + esc(e.at.slice(11, 19)) + "</td><td>" + (e.kind === "span" ? "<b>" + esc(e.name) + "</b>" : esc(e.name)) + (e.attempt ? " <span class='banner'>a" + e.attempt + "</span>" : "") + "</td><td>" + (e.status ? chip(e.status) : "—") + "</td><td class='mono'>" + esc(e.regulator || "") + "</td><td>" + esc(e.detail) + "</td></tr>").join("") + "</table>" : "<span class='empty'>no records yet</span>") + "</section>";
