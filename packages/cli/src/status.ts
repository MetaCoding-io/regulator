...
import {
  ExecutionStore, LEASES_RELATIVE_DIR, LeaseStore, checkRegistry, readSignals,
  type RegistryProblem,
} from "@metacoding/vsm-pi-core";
import {
  WorkloadDefinitionSchema, assertValid,
  type AttemptRecord, type Lease, type RegulatorRecord, type ResultReport, type UnitRecord, type VsmMessage, type WorkContract, type WorkloadDefinition,
...
export interface DefinitionView {
  ...
  declared: Array<"registry" | "workload">;
...
export async function readDefinition(dir: string): Promise<DefinitionView> {
  ...
  return {
    ...
    workloads,
    problems,
    declared: workloads.length ? ["registry", "workload"] : ["registry"],
...
export function renderStatusText(view: StatusView): string {
  ...
  if (view.instance) {
    ...
    for (const { unit, report, attemptRecords } of i.units) {
      const last = attemptRecords.at(-1);
      lines.push(`    ${unit.status.padEnd(10)} ${unit.unitId}  ${unit.unitType}  contract ${unit.contract.id} v${unit.contract.version}  attempts ${unit.attempts}${last ? ` (last: ${last.outcome})` : ""}${report ? "  reported" : ""}${unit.reason ? `  — ${unit.reason}` : ""}`);
    }
