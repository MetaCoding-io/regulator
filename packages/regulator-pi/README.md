...
`budget`, `recovery`, `evidence`, `authority`, `intelligence`, `identity`, `algedonic`,
loaded by `pi install ./packages/regulator-pi`), the dispatcher the control plane
resolves by name to run a unit's session, and the generic write gate and typed reporting
tools described below, which a Pi session can carry without a definition.

## The write gate

The write gate registers a typed `tool_call` handler for Pi's built-in `write` and
...
`pi -e ./packages/regulator-pi/dist/write-gate.js`. With that same Pi version already
available, the equivalent documented extension mechanism is:

```sh
pi -e ./packages/regulator-pi/dist/write-gate.js
```

For another project, run Pi from that project's root and pass an absolute path
to this repository's built `packages/regulator-pi/dist/write-gate.js`. `ctx.cwd` is
...
```sh
node packages/regulator-pi/dist/write-gate.test.js
```
