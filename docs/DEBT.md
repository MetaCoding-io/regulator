...
| 5 | Remediate, replan, clarify, pause and escalate are recorded and the unit waits; nothing performs them, reminds anyone, or delivers the algedonic signal beyond the read model. Policy proposals (lesson 10) land in the same sink with the same fate. | `recovery-router` limitations 2 and 4; `proposal-intake` limitation 1 | lesson 11 (obligation lifecycle), lesson 13 (algedonic delivery) | lesson 08 |
