...
report carries a fingerprint of the definition, the host pin and the suite, and a
person's interpretation; `regulator check` refuses a report under `evals/reports/` that
still carries the placeholder `regulator eval` writes without `--interpretation`. [Evidence about the
